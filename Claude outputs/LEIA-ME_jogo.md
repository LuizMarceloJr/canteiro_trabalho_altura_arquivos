...
Dê dois cliques em **`abrir_jogo.bat`**. Ele sobe um servidor local e abre
`http://localhost:8765/jogo/` no navegador. Feche a janela preta para encerrar.

> Abrir o `index.html` com dois cliques **não funciona**: o navegador bloqueia a leitura do
> modelo 3D em `file://`. Precisa ser por um servidor local — é o que o `.bat` faz.
>
> Sem Python na máquina: sirva a pasta `canteiro_trabalho_altura_arquivos` com qualquer
> servidor estático e abra `/jogo/`.

Requisitos: navegador com WebGL2 (Chrome, Edge ou Firefox atuais). Nada é instalado e nada sai
da máquina — inclusive o three.js e o decodificador Draco estão dentro de `jogo/vendor/`, então
o jogo roda **sem internet**.
